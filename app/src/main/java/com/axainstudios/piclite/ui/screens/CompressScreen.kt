...
import com.axainstudios.piclite.domain.TargetPreset
...
import com.axainstudios.piclite.ui.components.SelectableChip
...
fun CompressScreen(
    ...
    subtitle: String = "Reach an exact target size"
) {
    ...
    ScreenScaffold(
        ...
        LazyColumn(
            ...
            if (state.hasSources) {
                ...
                item { SectionLabel("Target size", Modifier.padding(top = 4.dp, start = 2.dp)) }
                item {
                    PresetGrid(selected = state.preset, gradient = g.hero, onSelect = vm::selectPreset)
                }
                item {
                    AnimatedVisibility(
                        visible = state.preset == TargetPreset.CUSTOM,
                        enter = expandVertically() + fadeIn(),
                        exit = shrinkVertically() + fadeOut()
                    ) {
                        CustomSizeField(value = state.customKb, onChange = vm::setCustomKb)
                    }
                }
                ...
                item {
                    GradientButton(
                        ...
                        else "Compress ${state.sources.size} photo${if (state.sources.size > 1) "s" else ""}",
                        gradient = g.hero,
...
private fun PresetGrid(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        TargetPreset.entries.chunked(3).forEach { row ->
...
@Composable
private fun CustomSizeField(value: String, onChange: (String) -> Unit) {
    OutlinedTextField(
        value = value,
